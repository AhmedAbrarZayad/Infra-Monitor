...
def anomaly_queryset(organization):
    return (
...
class AssistantContextView(APIView):
    def get(self, request, organization_id):
        ...
        anomalies = list(anomaly_queryset(organization)[:20])
        selected = None
        requested_id = request.query_params.get("anomaly_id")
        if requested_id:
            selected_id = parse_uuid(requested_id, "anomaly_id")
            selected = get_object_or_404(anomaly_queryset(organization), pk=selected_id)
...
class AssistantConversationCollectionView(APIView):
    def post(self, request, organization_id):
        ...
        anomaly = get_object_or_404(anomaly_queryset(organization), pk=anomaly_id)
...
def owned_conversation(request, organization, conversation_id):
...
class AssistantMessageListView(APIView):
    def get(self, request, organization_id, conversation_id):
        organization, _ = get_organization_membership(request, organization_id)
        conversation = owned_conversation(request, organization, conversation_id)
