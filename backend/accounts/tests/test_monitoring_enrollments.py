...
class MonitoringEnrollmentApiTests(TestCase):
    def setUp(self):
        ...
        self.owner = Users.objects.create_user(username="owner", email="owner@example.com", password="pass", is_email_verified=True)
        self.admin = Users.objects.create_user(username="admin", email="admin@example.com", password="pass", is_email_verified=True)
        self.engineer = Users.objects.create_user(username="engineer", email="engineer@example.com", password="pass", is_email_verified=True)
        self.outsider = Users.objects.create_user(username="outsider", email="outsider@example.com", password="pass", is_email_verified=True)
        ...
        OrganizationMembership.objects.create(organization=self.organization, user=self.owner, role="OWNER", approved=True)
        OrganizationMembership.objects.create(organization=self.organization, user=self.admin, role="ADMIN", approved=True)
        OrganizationMembership.objects.create(organization=self.organization, user=self.engineer, role="ENGINEER", approved=True)
    ...
    def test_owner_creates_enrollment_and_raw_token_is_not_stored(self):
        ...
        response = self.client.post(self.url, {"server_name": "Production API", "environment": "Production"}, format="json")
    ...
    def test_admin_can_create_but_engineer_cannot(self):
        ...
        self.assertEqual(self.client.post(self.url, {"server_name": "A", "environment": "dev"}).status_code, 201)
        self.client.force_authenticate(self.engineer)
        self.assertEqual(self.client.post(self.url, {"server_name": "B", "environment": "dev"}).status_code, 403)
    ...
    def test_list_is_organization_scoped_and_never_returns_token_material(self):
        ...
        EnrollmentToken.objects.create(organization=self.organization, created_by=self.owner, token_hash=TokenService.hash_enrollment_token(raw), token_prefix=raw[:15], server_name="Visible", environment="prod", expires_at=expiry)
        other_raw, other_expiry = TokenService.generate_enrollment_token()
        EnrollmentToken.objects.create(organization=self.other, created_by=self.outsider, token_hash=TokenService.hash_enrollment_token(other_raw), token_prefix=other_raw[:15], server_name="Hidden", environment="prod", expires_at=other_expiry)
        self.client.force_authenticate(self.owner)
